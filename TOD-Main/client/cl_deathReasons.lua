	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('Logs:PlayerDied', "`" .. GetPlayerName(PlayerId()) .. '` ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('Logs:PlayerDied', "`" .. GetPlayerName(Killer) .. '` ' .. DeathReason .. ' `' .. GetPlayerName(PlayerId()) .. '`.', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
