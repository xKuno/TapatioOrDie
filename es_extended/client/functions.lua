...
--[[ESX.ShowNotification = function(msg)
    exports['mythic_notify']:DoHudText('inform', msg, 5000,{ ['background-color'] = '#2F5C73', ['color'] = '#FFFFFF' })
    --print('performing mythic_notify from es_extended\\client\\functions.lua line 48')
end--]]
...
ESX.Game.spawnHandler = {}
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb)
	local model = (type(modelName) == 'number' and modelName or GetHashKey(modelName))

	Citizen.CreateThread(function()
		ESX.Game.spawnHandler[model] = cb
		TriggerServerEvent("AntiCheat:SpawnVehicles", model, coords.x, coords.y, coords.z, heading, true, false)
...
RegisterNetEvent("AntiCheat:vehicleCreated")
AddEventHandler("AntiCheat:vehicleCreated", function(netVeh, hash)
	print(hash)
    while not NetworkDoesEntityExistWithNetworkId(netVeh) do Wait(0); end
	...
	if ESX.Game.spawnHandler[hash] ~= nil then
		print("handling veh cb")
		ESX.Game.spawnHandler[hash](vehicle)
	end
end)
